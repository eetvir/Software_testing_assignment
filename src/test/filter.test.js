...
describe("filter", () => {
    it("should return empty array when empty array is used as input", () => {
        const array = []
    ...
    it("should return empty array if no elements match the predicate", () => {
        const array = [1, 3, 5]
        const predicate = (number) => number % 2 === 0
        expect(filter(array, predicate)).toEqual([[]])
    })

    it("filter arrays with duplicate values", () => {
        const array = [1, 2, 2, 6]
    ...
    it("should handle objects with a predicate", () => {
        const users = [
